...
class Likes {
  async createLike(l: Like): Promise<Like[]> {
    try {
      ...
      const like_SQL = `INSERT INTO likes (post_id, user_id, timedate) VALUES ($1, $2, $3) RETURNING *`;
      const user_SQL = `
  ...
  async checkLike(post_id: string, user_id: string): Promise<Like[] | null> {
    try {
      const connection = await database.connect();
  ...
  async deleteLike(l: Like): Promise<Like> {
    try {
      const connection = await database.connect();
      const like_SQL = 'DELETE FROM likes WHERE like_id=$1 RETURNING like_id';
      const like_result = await connection.query(like_SQL, [l.like_id]);
      connection.release();
      return like_result.rows[0];
    } catch (error) {
      throw new Error(
