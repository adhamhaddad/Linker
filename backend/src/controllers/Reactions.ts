...
const createReactions = async (req: Request, res: Response) => {
  try {
    const response = await reaction.createReactions(
      req.body,
      req.params.id,
      req.params.id
    );
...
const reactions_controller_routes = (
  app: Application,
  logger: NextFunction
) => {
  app.post('/user/:id/reactions', logger, createReactions);
  app.get('/user/:id/reactions', logger, getAllReactions);
  app.get('/user/:id/reactions', logger, getReactions);
  app.patch('/user/:id/reactions', logger, updateReactions);
  app.delete('/user/:id/reactions', logger, deleteReactions);
};
